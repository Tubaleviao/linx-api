...
let all_funcs = ['LinxClientesFornec', 'LinxLojas', 'LinxMovimento', 'LinxPlanos', 'LinxMovimentoPlanos',
  ...
  'LinxProdutosInventario', 'LinxProdutosDetalhes']
    ...
    /*['LinxClientesFornec', 'LinxLojas', 'LinxMovimento', 'LinxPlanos', 'LinxMovimentoPlanos',
  'LinxMovimentoAcoesPromocionais', 'LinxVendedores', 'LinxAcoesPromocionais', 'LinxProdutos',
  'LinxProdutosInventario', 'LinxProdutosDetalhes']*/
let cnpjs = []
let executions = []
let dates = [];
//const from = new Date('2019-04-08') // inclusive pulei 2018-02-09 19/02/2018
//const to = new Date('2019-04-08') // inclusive 
...
// GET DATA
var getData = async (APIname='', parameters={}, insertFunction=false, deleteStr=false, deleteAfter=false, deleteAfter2=false, dt=false) => {
  ...
  if( APIname === 'LinxGrupoLojas'){
    ...
    if(csv_array.length === 0){
      ...
      process.stdout.write(new_csv_array.length+' - '+dt)
      console.time(APIname);
      let values = [];
      
      new_csv_array.forEach((a,b)=>{
        values[Math.floor(b/1000)] ? true : values[Math.floor(b/1000)]='';
        let res = insertFunction(a)
        values[Math.floor(b/1000)] += (res ? res : '');
      });
      ...
      for(let idx = 0; idx < values.length; idx++){
        ...
        let inserted;
        if(rec){
           inserted = await insert("insert into "+APIname.replace('Linx', '')+" values "+rec.substr(1, rec.length-1)+";")
        }
        process.stdout.write(' ~inserting~ ');
        if(!inserted){console.log(rec)}
      }
      if(deleteAfter){
      ...
      if(deleteAfter2){
        let deleted2 = await insert(deleteAfter2)
...
var next = function(){
  ...
  if(fn){
    getData( fn[0], fn[1], fn[2], fn[3], fn[4], fn[5], fn[6] )
  }
}
      ...
      all_funcs.forEach( apiName => {
        ...
        switch(apiName){
          case 'LinxProdutosInventario':
            deleteStr = `cnpj_emp = '${params.cnpjEmp}';`
            insertFunction = json => {
              let res = ',('+((json.portal==='') ? 'null' : json.portal.replace(',','.'))+','+'\''+json.cnpj_emp+'\','+((json.cod_produto==='') ? 'null' : json.cod_produto.replace(',','.'))+','+'\''+json.cod_barra+'\','+((json.quantidade==='') ? 'null' : json.quantidade.replace(',','.'))+')'
              return res;
            }
            params = {...params, data_inventario: dia[0]}
            executions.push([apiName, params, insertFunction, deleteStr, deleteAfter, deleteAfter2, string_yest]);
          break;
          case 'LinxProdutos':
          ...
          case 'LinxMovimento':
            ...
            deleteAfter2 = `delete from Movimento where exists(
                            select b.cod_barra, b.cnpj_emp, b.chave_nf, b.portal, 
                              b.documento, b.data_lancamento, b.dt_update, b.transacao, count(1)
                            from Movimento b where excluido = 'S'
                              and b.cod_barra=Movimento.cod_barra and b.cnpj_emp=Movimento.cnpj_emp and b.chave_nf=Movimento.chave_nf 
                              and b.portal=Movimento.portal and b.documento=Movimento.documento and b.data_lancamento=Movimento.data_lancamento
                              and	b.transacao=Movimento.transacao
                            group by b.cod_barra, b.cnpj_emp, b.chave_nf, b.portal, 
                              b.documento, b.data_lancamento, b.dt_update, b.transacao
                          )`
            params = {...params, data_inicial: dia[0], data_fim: dia[1]}
            executions.push([apiName, params, insertFunction, deleteStr, deleteAfter, deleteAfter2, string_yest]);
          break;
...
let insert = (sql, table='') => {
  return new Promise((resolve, reject) => {
    let db_conn = new SqlConn(db_config)
    db_conn.on('connect', (err) => {if (!err){
      let req_in = new SqlReq(sql, (err, rowCount, rows) => {
        db_conn.close()
